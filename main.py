...
---------------------
The application currently lives in a single file. A modular split into
``constants``, ``audio``, ``ui/`` and future ``voice/`` packages is planned
once the feature set stabilises (see project roadmap).
...
# Auto-updater — increment APP_VERSION before each release, then push a
# matching git tag (e.g. v1.1.0). The GitHub Actions workflow verifies
# that the tag and this constant are in sync before building.
APP_VERSION = "1.0.0"
...
# then deletes itself. ``close_fn`` must destroy the Tkinter window so the
# OS releases the file lock before the .bat runs.
# ---------------------------------------------------------------------------
...
def _apply_update(close_fn, new_exe: str) -> None:
    ...
    than the app object) keeps this function usable from both the splash and,
    hypothetically, any other window without importing FrenchTTSApp.
    """
    ...
    def _check_worker(self) -> None:
        ...
        if _version_tuple(tag) > _version_tuple(APP_VERSION):
            asset = next(
                ...
                None)
            if asset is None:
                # Release exists but the exe asset is missing — retrying won't help
                # immediately, but the maintainer may re-upload, so we still offer retry.
                self.after(0, lambda: self._show_error(
...
def main():
    ...
    # Note: in --update dev mode the .bat swap is not executed even if a newer
    # version is found — _apply_update writes the .bat only when sys.frozen is
    # True, because sys.executable would point to the Python interpreter otherwise.
